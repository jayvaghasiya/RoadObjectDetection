...
CLAZZ_NUMBERS = {KEY_PEDESTRIAN: 0, KEY_CYCLIST: 1, KEY_CAR: 2, KEY_DONT_CARE: 3}
...
def parseArguments():
    parser = argparse.ArgumentParser(
        description="Generates labels for training darknet on KITTI."
    )
    parser.add_argument(
        "label_dir",
        help="data_object_label_2/training/label_2 directory; can be downloaded from KITTI.",
    )
    parser.add_argument(
        "image_2_dir",
        help="data_object_image_2/training/image_2 directory; can be downloaded from KITTI.",
    )
    parser.add_argument(
        "--training-samples",
        type=int,
        default=0.8,
        help="percentage of the samples to be used for training between 0.0 and 1.0.",
    )
    parser.add_argument(
        "--use-dont-care", action="store_true", help="do not ignore 'DontCare' labels."
    )
    args = parser.parse_args()
    if args.training_samples < 0 or args.training_samples > 1:
        print(
            "Invalid argument {} for --training-samples. Expected a percentage value between 0.0 and 1.0."
        )
        exit(-1)
    return args
    ...
    for dir_path, sub_dirs, files in os.walk(args.label_dir):
        for file_name in files:
            if file_name.endswith(".txt"):
                ...
                with open(
                    os.path.join(OUT_LABELS_DIR, "{}.txt".format(sample_id)), "w"
                ) as yolo_label_file:
